-- L1 hero corpus: standalone procedure referencing customers.legacy_segment.
-- PLSQL-LAB-008 / §1.4 DROP COLUMN hero scenario.
--
-- A standalone procedure (third dependent unit) so DROP COLUMN yields
-- at least three distinct INVALID objects: view + package body + procedure.
--
...
IS
    ...
    v_count  NUMBER;
BEGIN
    -- Summarise legacy_segment distribution, optionally filtered by region.
    FOR rec IN (
