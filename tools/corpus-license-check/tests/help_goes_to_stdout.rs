...
fn help_flag_writes_to_stdout_not_stderr() {
    let bin = env!("CARGO_BIN_EXE_corpus-license-check");
    let out = Command::new(bin)
        .arg("--help")
        .output()
        .expect("spawn corpus-license-check --help");
...
fn short_help_flag_writes_to_stdout_not_stderr() {
    let bin = env!("CARGO_BIN_EXE_corpus-license-check");
    let out = Command::new(bin).arg("-h").output().expect("spawn corpus-license-check -h");
    assert_eq!(out.status.code(), Some(0));
    let stdout = String::from_utf8_lossy(&out.stdout);
    let stderr = String::from_utf8_lossy(&out.stderr);
