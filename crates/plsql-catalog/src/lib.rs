...
#[cfg(feature = "oraclemcp-db")]
pub use live::{
    OracleConnection, OraclemcpDbConnection, fetch_dbms_metadata_ddl,
    load_snapshot_from_connection, negotiate_capabilities, populate_dbms_metadata_ddl,
};
...
mod tests {
    ...
    use crate::live::{LiveContext, RuntimeBuilder, load_catalog_users};
    use crate::{
        ...
        grantee_from_dictionary_value, load_from_dbms_metadata_dir, load_snapshot_from_connection,
        load_snapshot_from_json, negotiate_capabilities, populate_dbms_metadata_ddl,
    };
    ...
    impl OracleConnection for StaticConnection {
        ...
        async fn ping(&self, cx: &LiveContext) -> Result<(), CatalogError> {
            let _ = cx;
            Ok(())
        }

        async fn describe(&self, cx: &LiveContext) -> Result<OracleConnectionInfo, CatalogError> {
            let _ = cx;
    ...
    fn run_catalog_future<F: Future>(future: F) -> F::Output {
        RuntimeBuilder::current_thread()
            .build()
            .expect("test live runtime")
            .block_on(future)
    }

    fn test_cx() -> LiveContext {
        LiveContext::current().expect("test runtime installs a request context")
    }
