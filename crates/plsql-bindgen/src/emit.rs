...
pub fn emit_wrappers(plan: &BindingPlan) -> String {
    ...
    let _ = writeln!(
        ...
        comment_safe(&plan.package_name),
        plan.routines.len()
    );
...
/// Fold every control character (notably `\n` / `\r`, but also any other
/// `char::is_control()` byte) in `s` to a single space, returning a one-line,
/// comment-safe copy.
///
/// Several emitter sites interpolate UNTRUSTED [`BindingPlan`] strings —
/// `package_name` (settable via the `--package` CLI override) and every
/// [`BindingDiagnostic`] field (`routine` / `code` / `message` /
/// `suggested_workaround`) — raw into single-line `//` comments. A `\n` in any
/// of those terminates the comment, so the remainder of the field would be
/// emitted as live top-level Rust into the generated module, violating the
/// emitter's documented fail-closed "always emit compilable/safe Rust"
/// invariant (and at worst smuggling code that compiles into a consuming
/// crate). Routing each field through `comment_safe` keeps the whole comment on
/// one line so no field can break out of it.
...
fn emit_diagnostic_comment(out: &mut String, diag: &BindingDiagnostic) {
    let scope = diag
        .routine
        .as_deref()
        .map(|r| format!("[{}] ", comment_safe(r)))
        .unwrap_or_default();
    let _ = writeln!(
        out,
        "// bindgen-diag {scope}({}): {}",
        comment_safe(&diag.code),
        comment_safe(&diag.message)
    );
    if let Some(workaround) = &diag.suggested_workaround {
        let _ = writeln!(out, "//   workaround: {}", comment_safe(workaround));
    }
}
...
mod tests {
    ...
    /// Assert that every non-blank line of `src` is a `//` comment line — i.e.
    /// no field smuggled a top-level Rust item out of a `//` comment. Used by
    /// the header/diagnostic CR/LF-injection regression tests where the plan
    /// carries NO routines, so the only legitimate output is comment text.
    fn assert_all_lines_are_comments(src: &str) {
        for line in src.lines() {
            let trimmed = line.trim_start();
            assert!(
                trimmed.is_empty() || trimmed.starts_with("//"),
                "a non-comment line leaked into generated output (CR/LF injection): \
                 {line:?}\nfull source:\n{src}"
            );
        }
    }
    ...
    #[test]
    fn package_name_with_newline_cannot_escape_header_comment() {
        // oracle-73t1.2: package_name is interpolated raw into the single-line
        // `// Generated by ...` header. A `\n` (settable via the untrusted
        // --package override) would terminate the comment and emit the rest as
        // live top-level Rust. With comment_safe the whole header stays on one
        // line, so the injected `}`/`fn`/`use` items can never escape.
        let mut p = plan(vec![]);
        // VARIANT 1 (denial-of-build) + VARIANT 2 (token smuggling) combined.
        p.package_name =
            "demo\n}\nfn injected_top_level_item() { let x = 1; }\nmod m { \npub fn injected() -> i32 { 42 }\n//"
                .into();
        let src = emit_wrappers(&p);
        assert_all_lines_are_comments(&src);
        // The injected tokens survive only as inert comment text, never as a
        // top-level item: no line *starts* with a smuggled `fn`/`mod`/`pub`/`}`.
        assert!(
            !src.contains("\nfn injected_top_level_item"),
            "an injected `fn` reached a top-level position: {src}"
        );
        assert!(
            !src.contains("\npub fn injected"),
            "an injected `pub fn` reached a top-level position: {src}"
        );
        assert!(
            !src.contains("\nmod m"),
            "an injected `mod` reached a top-level position: {src}"
        );
    }

    #[test]
    fn diagnostic_fields_with_newlines_cannot_escape_comment() {
        // oracle-73t1.4: every BindingDiagnostic field (routine/code/message/
        // suggested_workaround) is interpolated raw into a single-line `//`
        // comment. A `\n` in any of them terminates the comment and emits the
        // remainder as live top-level Rust. comment_safe folds control chars to
        // spaces so every line stays a `//` comment. No routines → the only
        // legitimate output is comment text.
        let mut p = plan(vec![]);
        p.diagnostics.push(BindingDiagnostic {
            routine: Some("foo\nstruct InjViaRoutine;\n//".into()),
            code: "BG_X\npub fn pwned() { std::process::exit(0); }\n//".into(),
            message: "benign text\npub struct AlsoInjected;\n//".into(),
            suggested_workaround: Some(
                "do this\nimpl AlsoInjected { pub fn via_workaround() -> u32 { 7 } }\n//".into(),
            ),
            span: None,
        ...
        let src = emit_wrappers(&p);
        assert_all_lines_are_comments(&src);
        // None of the injected items reaches a line-leading top-level position.
        assert!(
            !src.contains("\npub fn pwned"),
            "code field smuggled a `pub fn`: {src}"
        );
        assert!(
            !src.contains("\npub struct AlsoInjected"),
            "message field smuggled a `pub struct`: {src}"
        );
        assert!(
            !src.contains("\nimpl AlsoInjected"),
            "workaround field smuggled an `impl`: {src}"
        );
        assert!(
            !src.contains("\nstruct InjViaRoutine"),
            "routine field smuggled a `struct`: {src}"
        );
        // The diagnostic is still emitted (folded onto one line), not dropped.
        assert!(src.contains("bindgen-diag"), "{src}");
    }
